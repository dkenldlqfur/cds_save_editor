...
- **주인공 정보**
  ...
  - 스폰서 계약과 남은 일수 확인·해제
  - 성격 계산 팝업: 별자리·직업·얼굴·나이 보정을 8개 성격축별로 표시
- **인물 정보**
...
성격은 세이브에 고정값으로 보관되지 않으며, 게임이 아래 식으로 8개 축을 계산합니다.

```text
결과 = 제한(별자리 기본값 + 직업 보정 + 얼굴/나이 보정, 0, 2)
```

- 36세 이상이면 얼굴 보정에 사용하는 코드가 변경됩니다.
- 각 축의 `0`은 왼쪽 성향, `1`은 중립, `2`는 오른쪽 성향입니다.
- 주인공 정보의 **성격** 버튼에서 현재 세이브 값으로 계산한 모든 항목을 확인할 수 있습니다.
