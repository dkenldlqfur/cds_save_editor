...
EMPTY_ROLE_SLOT = 65535
ROLE_CHARACTER_PREFIX = 4096
ROLE_CHARACTER_MASK = 65280

def role_slot_code(character_id):
    """인물 ID를 세이브의 역할 슬롯 16비트 코드로 변환한다."""
    character_id = int(character_id)
    if not 0 <= character_id <= 255:
        raise ValueError(error_text('role_character_id_out_of_range'))
    return ROLE_CHARACTER_PREFIX | character_id

def role_character_id(slot_code):
    """역할 슬롯 코드에서 인물 ID를 읽고 빈/비인물 코드는 None으로 반환한다."""
    slot_code = int(slot_code) & 65535
    return slot_code & 255 if slot_code & ROLE_CHARACTER_MASK == ROLE_CHARACTER_PREFIX else None

def read_role_character_id(buffer, offset):
...
def write_role_character_id(buffer, offset, character_id):
...
def active_role_character_ids(buffer, role_offsets):
    """여러 역할 슬롯에서 현재 배정된 인물 ID 집합을 반환한다."""
    return frozenset((character_id for offset in role_offsets for character_id in (read_role_character_id(buffer, offset),) if character_id is not None))
