...
def refreshed_ship_mask(base_mask, candidate_mask, city_scale, base_year, target_year, release_coefficients):
    """도시 조선소의 연도별 판매 후보 마스크를 계산한다.

    게임은 각 연도에 도시 후보 선박 중 출시 조건을 충족한 번호가 가장 높은 한 종을
    추가한다. 날짜를 앞으로 건너뛴 편집에서도 원본 연도부터 누적해 같은 결과를 낸다.
    """
    result = int(base_mask) & 65535
    candidates = int(candidate_mask) & 65535
    ...
    for year in range(int(base_year), int(target_year) + 1):
        ...
        candidate = max((ship_code for ship_code, coefficient in enumerate(coefficients) if candidates & 1 << ship_code and coefficient < threshold), default=-1)
        if candidate >= 0:
            result |= 1 << candidate
    return result
