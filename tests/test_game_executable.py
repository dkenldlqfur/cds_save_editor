import struct
import unittest
from editor_core.game_executable import CHARACTER_RECORD_COUNT, CHARACTER_RECORD_SIZE, CHARACTER_TABLE_FILE_OFFSET, CITY_RECORD_COUNT, CITY_RECORD_SIZE, CITY_TABLE_VA, FIGUREHEAD_EFFECT_CONFIG_OFFSET, FIGUREHEAD_EFFECT_DEFAULT_VALUES, FIGUREHEAD_EFFECT_MAGIC, FIGUREHEAD_EFFECT_HOOK_TARGETS, FIGUREHEAD_EFFECT_VERSION, ITEM_NO_IMAGE_ID, ITEM_RECORD_COUNT, ITEM_RECORD_SIZE, ITEM_TABLE_VA, LIMIT_OPERANDS, SHIP_TYPE_RECORD_COUNT, SHIP_TYPE_RECORD_SIZE, SHIP_TYPE_TABLE_VA, SPONSOR_RECORD_COUNT, SPONSOR_RECORD_SIZE, SPONSOR_TABLE_VA, TRADE_GOOD_RECORD_COUNT, TRADE_GOOD_RECORD_SIZE, TRADE_GOOD_TABLE_VA, TRADE_REGION_COUNT, TRADE_REGION_GOODS_PER_REGION, TRADE_REGION_GOODS_TABLE_VA, _read_characters, _city_profile_mappings, _read_cities, _read_figurehead_effects, _read_items, _read_limits, _read_ship_types, _read_sponsors, _read_trade_regions
...
class _SponsorImage:
    ...
    def __init__(self):
        ...
        self.data = bytearray(self.table_offset + SPONSOR_RECORD_COUNT * SPONSOR_RECORD_SIZE)
        self.names = {}
        expected_names = ['조안 2세', '조안 1세']
        expected_names.extend(('후원자 {0}'.format(index) for index in range(2, SPONSOR_RECORD_COUNT)))
        for identifier, name in enumerate(expected_names):
            offset = self.table_offset + identifier * SPONSOR_RECORD_SIZE
            name_pointer = 6291456 + identifier * 32
            struct.pack_into('<I', self.data, offset - 4, name_pointer)
            struct.pack_into('<14I', self.data, offset, identifier, identifier % 2, 0, 0, 0, 0, 0, 100, 0, 0, 100, 100, 0, 0)
...
class _SponsorFallback:
    ...
    def __init__(self):
        self.sponsor_by_id = {identifier: {'id': identifier, 'name': '기본 {0}'.format(identifier)} for identifier in range(SPONSOR_RECORD_COUNT)}

class _CharacterImage:

    def __init__(self):
        self.data = bytearray(CHARACTER_TABLE_FILE_OFFSET + CHARACTER_RECORD_COUNT * CHARACTER_RECORD_SIZE)
        self.names = {}
        for identifier in range(CHARACTER_RECORD_COUNT):
            offset = CHARACTER_TABLE_FILE_OFFSET + identifier * CHARACTER_RECORD_SIZE
            first_pointer = 6291456 + identifier * 32
            ...
            self.names[first_pointer] = '이름{0}'.format(identifier)
...
class _CityImage:
    ...
    def __init__(self):
        ...
        for identifier in range(CITY_RECORD_COUNT):
            ...
            values[24] = 9 << 16 | 62587
            struct.pack_into('<34i', self.data, offset, *values)
            self.names[name_pointer] = '도시 {0}'.format(identifier)
...
class _ShipTypeImage:
    ...
    def __init__(self):
        self.table_offset = 32
        self.data = bytearray(self.table_offset + SHIP_TYPE_RECORD_COUNT * SHIP_TYPE_RECORD_SIZE)
        self.names = {}
        for identifier in range(SHIP_TYPE_RECORD_COUNT):
            offset = self.table_offset + identifier * SHIP_TYPE_RECORD_SIZE
            name_pointer = 7405568 + identifier * 32
            values = [name_pointer, 0, 10 + identifier, 20, 30, 40, 50, 60, 70, 80, 90, 5, 10, identifier, 100 + identifier, 1 + identifier]
            struct.pack_into('<16I', self.data, offset, *values)
            self.names[name_pointer] = '선종 {0}'.format(identifier)
...
class _ItemImage:
    ...
    def __init__(self):
        ...
        self.data = bytearray(self.table_offset + ITEM_RECORD_COUNT * ITEM_RECORD_SIZE)
        self.names = {}
        for identifier in range(ITEM_RECORD_COUNT):
            offset = self.table_offset + identifier * ITEM_RECORD_SIZE
            name_pointer = 7471104 + identifier * 32
            ...
            struct.pack_into('<7I', self.data, offset, name_pointer, image_id, 200 + identifier, 100 + identifier, identifier & 255, category_id, 0)
            self.names[name_pointer] = '아이템 {0}'.format(identifier)
...
class _TradeRegionImage:
    ...
    def __init__(self):
        ...
        self.goods_table_offset = self.price_table_offset + TRADE_GOOD_RECORD_COUNT * TRADE_GOOD_RECORD_SIZE + 64
        self.data = bytearray(self.goods_table_offset + TRADE_REGION_COUNT * TRADE_REGION_GOODS_PER_REGION * 4)
        for good_id in range(TRADE_GOOD_RECORD_COUNT):
            for region_id in range(TRADE_REGION_COUNT):
                struct.pack_into('<i', self.data, self.price_table_offset + good_id * TRADE_GOOD_RECORD_SIZE + region_id * 4, 1000 + good_id * 100 + region_id)
        for region_id in range(TRADE_REGION_COUNT):
            goods = [region_id, (region_id + 1) % TRADE_GOOD_RECORD_COUNT, -1, -1, -1]
            struct.pack_into('<{0}i'.format(TRADE_REGION_GOODS_PER_REGION), self.data, self.goods_table_offset + region_id * TRADE_REGION_GOODS_PER_REGION * 4, *goods)
...
class _FigureheadEffectImage:
    ...
    def __init__(self, values=None):
        ...
        self.slot_va = 6291456
        self.hook_offsets = {hook_va: 160 + index * 8 for index, (hook_va, _stub_offset, _opcode) in enumerate(FIGUREHEAD_EFFECT_HOOK_TARGETS)}
        if values is not None:
            ...
            struct.pack_into('<{0}I'.format(len(values)), self.data, offset + FIGUREHEAD_EFFECT_CONFIG_OFFSET, *values)
            for hook_va, stub_offset, opcode in FIGUREHEAD_EFFECT_HOOK_TARGETS:
                ...
                struct.pack_into('<i', self.data, hook_offset + 1, self.slot_va + stub_offset - (hook_va + 5))
...
class GameExecutableCityTests(unittest.TestCase):
    ...
    def test_city_basic_defaults_are_read_from_executable(self):
        ...
        self.assertEqual(62587, city['facility_flags'])
    ...
    def test_city_profile_lookups_follow_each_city_record(self):
        ...
        self.assertEqual([126, 2], mappings['ship_candidate_mask_runs'][0])
